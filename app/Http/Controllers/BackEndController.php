...
public function deletequalification($id)
    {
       // dd($id);
        try {
            DB::table('teammember_document_files')->where(['id'=>$id,
      ...
      //  dd($request->id); die;
        if ($request->ajax()) {
            ...
            //   dd($request->id);
              ...
              ->select('teammembers.mobile_no')->
              where('teammembers.id',$request->id)->first();
...
if ($mentees == null) {
    ...
    ->get();
	   ...
	   if(auth()->user()->role_id == 11 || auth()->user()->role_id == 12){
        ...
        ->select('notifications.*','teammembers.profilepic','teammembers.team_member'
    ...
    ->select('assettickets.*','teammembers.team_member')->orderBy('created_at','desc')->paginate(2);
     ...
     ->leftjoin('clients','clients.id','assignmentbudgetings.client_id')
      ->select('assignmentbudgetings.*','clients.client_name',
     'assignments.assignment_name')->orderBy('assignmentbudgetings.created_at','desc')->take(3)->get();
 ...
 ->leftjoin('assignmentteammappings','assignmentteammappings.assignmentmapping_id','assignmentmappings.id')
 ->leftjoin('assignments','assignments.id','assignmentmappings.assignment_id')
 ->select('assignmentbudgetings.client_id','clients.client_name','assignmentbudgetings.assignmentgenerate_id',
 'assignments.assignment_name','assignmentbudgetings.assignmentname')
        ...
        //    //   dd($teammemberid);
   	  ...
   	  if($request->document_file != null){
           ...
           for ($i = 0; $i < count($qualifications); $i++) {
               ...
               if ($documentFile) {
                   $documentFileName = $documentFile->getClientOriginalName();
