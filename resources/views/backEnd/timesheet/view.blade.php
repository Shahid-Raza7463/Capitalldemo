                     ...
                     <div class="card-body">
                        ...
                        data-toggle="modal" data-target="#exampleModal12"
                            class="btn btn-info-soft btn-sm">Mail <i class="far fa-envelope"></i></a> -->
                             <b style="float:right;margin-top: -17px;"><a
                                     href="{{ route('timesheet.edit', $timesheet->id) }}" class="btn btn-info-soft btn-sm">
                             ...
                             <table class="table display table-bordered table-striped table-hover">
                                 ...
                                 <tbody>
                                     ...
                                     <tr>
                                         ...
                                         @php
                                             $timesheetData = DB::table('timesheets')
                                                 ...
                                                 ->select('timesheets.*', 'clients.client_name')
                                                 ->get();

                                         @endphp
                                         <td>
                                             @foreach ($timesheetData as $timesheetDatas)
                                                 {{ $timesheetDatas->client_name ?? '' }}
                                             @endforeach
                                         </td>
                                         ...
                                         @php
                                             $timesheetData = DB::table('timesheets')
                                                 ->leftjoin('assignments', 'assignments.id', 'timesheets.assignment_id')
                                                 ->where('timesheets.id', $timesheet->id)
                                                 ->select('timesheets.*', 'assignments.assignment_name')
                                                 ->get();
                                     ...
                                     <tr>
                                         ...
                                         <td><b></b>Createdby</td>
                                         <td>{{ App\Models\Teammember::select('team_member')->where('id', $timesheet->createdby)->first()->team_member ?? '' }}
                                         </td>
                                     </tr>
                                 </tbody>
                             </table>
     ...
     <div class="modal fade" id="exampleModal12" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel4"
         ...
         <div class="modal-dialog" role="document">
             <div class="modal-content">
                 <form id="detailsForm" method="post" action="{{ url('/task/reminder') }}" enctype="multipart/form-data">
                     @csrf
                     ...
                     <div class="modal-body">
                         ...
                         <div class="row row-sm">
                             ...
                             <div class="col-sm-6">
                                 <input placeholder=" Enter Subject" value="Kgs Task Reminder" class="form-control"
                                     name="subject" type="text">
                                 <input hidden placeholder=" Enter Subject" value="{{ $id }}"
                                     class="form-control" name="taskid" type="text">
                             </div>
                             <div class="col-sm-6">
                                 <select class="form-control basic-multiple" multiple="multiple" name="email[]">
                                     @if (Request::is('view/task/*'))
                                         @foreach ($taskassign as $taskassignData)
                                             @foreach ($teammember as $teammemberData)
                                                 <option value="{{ $teammemberData->emailid }}"
                                                     @if ($teammemberData->id == $taskassignData->teammember_id) selected @endif>
                         ...
                         <div class="row row-sm">
                             <div class="col-sm-12">
                                 <textarea rows="6" name="msg" class="centered form-control" id="editor" placeholder="Enter Description">
                                Dear Sir/Madam
