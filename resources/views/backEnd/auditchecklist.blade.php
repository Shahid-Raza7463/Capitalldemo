    ...
    <div class="content-header row align-items-center m-0">
        ...
        <nav aria-label="breadcrumb" class="col-sm-6 order-sm-last mb-3 mb-sm-0 p-0 ">
            <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
                @php
                    $status = App\Models\Checklistanswer::leftJoin('statuses', function ($join) {
                        ...
                        ->where('assignment_id', $assignmentbudget->assignmentgenerate_id)
                        ->select('statuses.*')
                        ->orderBy('id', 'asc')
                        ->first();
                    $count = App\Models\Checklistanswer::leftJoin('statuses', function ($join) {
                        $join->on('checklistanswers.status', 'statuses.id');
                    })
                        ->where('steplist_id', $stepname->id)
                        ->where('financialstatemantclassfication_id', $financialname->id)
    ...
    <div class="body-content">
        <div class="row">
                                                        ...
                                                        @if ($authteamtl != null || $authteamid != null)
                                                            <th><button type="submit"
                                                                ...
                                                                <input type="text" hidden name="steplist_id"
                                                                    value="{{ $stepname->id }}">
                                                                <input type="text" hidden name="assignment_id"
                                                                    value="{{ $assignmentbudget->assignmentgenerate_id }}">
                                                                <input type="text" hidden name="subclassfied_id"
                                                                    value="{{ $subclassficationname->id }}">
                                                                <input type="text" hidden name="financialid"
                                                                    value=" {{ $subclassficationname->financialstatemantclassfication_id }}">
                                                                @if ($authteamid != null)
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($auditprocedure as $sub => $value)
                                                <tr>
                                                        ...
                                                        @if (Auth::user()->role_id == 15 || Auth::user()->role_id == 14)
                                                            @if ($authteamtl != null || $authteamid != null)
                                                                <td><input type="checkbox" name="ids[]"
                                                                        style="width: 18px;margin-left: 16px;"
                                                    ...
                                                    <td> {{ $sub + 1 }}</td>
                                                    <td> <a
                                                            href="{{ url('/auditchecklistanswer?' . 'auditid=' . $value->id . '&&' . 'assignmentid=' . $assignmentbudget->assignmentgenerate_id) }}">{{ $value->auditprocedure }}</a>
                                                    </td>
            ...
            ->where('assignment_id',$assignmentbudget->assignmentgenerate_id)->get() as $status)
                                                            <span class="{{ $status->color }}">{{ $status->name }}</span>
                                                            <span><i
                                                                    class="typcn typcn-media-record text-success"></i></span>
                                                        @empty
                                                            <span class="badge badge-primary">OPEN</span>
                                                        @endforelse
                                                    </td>
                                                    <td> {{ DB::table('audittrails')->leftjoin('checklistanswers', 'checklistanswers.id', 'audittrails.auditanswer_id')->leftjoin('teammembers', 'teammembers.id', 'audittrails.created_by')->where('checklistanswers.assignment_id', $assignmentbudget->assignmentgenerate_id)->where('checklistanswers.audit_id', $value->id)->select('teammembers.team_member', 'audittrails.id')->orderBy('id', 'DESC')->pluck('teammembers.team_member')->first() ?? '' }}
                                                    </td>
                                                    @php
                                                        $critical = App\Models\Criticalnote::where('assignmentgenerateid', $assignmentbudget->assignmentgenerate_id)
                                                            ->where('auditquestionid', $value->id)
                                                            ->first();
                                                    @endphp
                                                    @if ($critical == null)
                                                        <td> </td>
                                                    @else
                                                        <td> <a href="{{ url('/criticalnotes?' . 'auditid=' . $value->id . '&&' . 'assignmentid=' . $assignmentbudget->assignmentgenerate_id) }}"
                                                                class="btn btn-success-soft btn-sm mr-1"><i
...
<div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel4"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form id="detailsForm" method="post" action="{{ url('checklist/upload') }}"
                ...
                <div class="modal-body">
                    ...
                    <div class="details-form-field form-group row">
                        ...
                        <div class="col-sm-12">
                            <input required class="form-control" name="checklist" type="text"
                                placeholder="Enter Question">
                            <input class="form-control" hidden name="financialid"
                                value="{{ $financialname->id ?? '' }}" type="text">
